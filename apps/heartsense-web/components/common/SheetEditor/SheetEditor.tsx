...
const PureSpreadsheetEditor = ({ content, saveContent }: SheetEditorProps) => {
  ...
  const parseData = useMemo(() => {
    if (!content)
      return Array.from({ length: MIN_ROWS }).fill(
        Array.from({ length: MIN_COLS }).fill('')
      )
    ...
    while (paddedData.length < MIN_ROWS) {
      paddedData.push(Array.from({ length: MIN_COLS }).fill('') as string[])
    }
    ...
    () =>
      parseData.map((row, rowIndex) => {
        ...
        for (const [colIndex, col] of columns.slice(1).entries()) {
          rowData[col.key] = row[colIndex as keyof typeof row] || ''
        }
...
export function areEqualSheetEditor(
...
export const SpreadsheetEditor = memo(
  PureSpreadsheetEditor,
  areEqualSheetEditor
)
